    ...
    <article class="staff-reports-panel">
      ...
      <div class="staff-report-grid">
        <div v-for="report in reports.reports || []" :key="report.id" class="staff-report" :class="report.urgency">
          ...
          <p>{{report.detail}}</p><footer><em>{{report.confidence_label}} · {{report.confidence}}% confianza</em><b>{{report.evidence}}</b><button type="button" class="football-button tiny" @click="emit('open-action',report.action)">Abrir</button></footer>
        </div>
      </div>
    </article>
      ...
      <section v-for="group in areas" :key="group.area" class="staff-area">
        ...
        <div class="staff-responsibility" v-for="row in group.rows" :key="row.key">
          ...
          <div class="responsibility-quality" :class="qualityClass(row.quality)">
            ...
            <button type="button" class="football-button tiny secondary responsibility-open" @click="emit('open-action',row.workspace || 'home')">Abrir área</button>
          </div>
        </div>
      </section>
    </div>
