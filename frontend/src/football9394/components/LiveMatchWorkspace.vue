...
const historyContext = computed(() => opponent.value?.history || [])
const reencounterContext = computed(() => opponent.value?.reencounters || [])
const rivalryHeat = computed(() => Number(opponent.value?.rivalry?.heat || 0))
...
const roundSummary = computed(() => props.match?.round_summary || null)
const roundResults = computed(() => roundSummary.value?.results || [])
const postContext = computed(() => props.match?.postmatch_context || {})
const postNextMatch = computed(() => postContext.value?.next_match || props.nextMatch || null)
const nextAbsences = computed(() => postContext.value?.next_match_absences || [])
const nextOpponentName = computed(() => {
  if(!postNextMatch.value)return 'Por confirmar'
  const controlled=Number(props.match?.controlled_team_id||0)
  return Number(postNextMatch.value.home_team_id||0)===controlled ? (postNextMatch.value.away_team || 'Rival') : (postNextMatch.value.home_team || 'Rival')
})
const postPosition = computed(() => {
  const row=postContext.value?.standings || {}
  if(row.position)return `${row.position}º · ${row.points ?? 0} pts`
  return props.dashboard?.position ? `${props.dashboard.position}º · ${props.dashboard.points ?? 0} pts` : `${props.dashboard?.points ?? 0} pts`
})
      ...
      <div class="live-layout">
        <article class="football-panel live-commentary modern-commentary" :class="{ preflight: match.minute===0, finished: match.status==='finished' }">
          ...
          <div v-if="match.minute===0" class="match-preview-v2 d7-match-preview">
            ...
            <div v-if="historyContext.length || reencounterContext.length || rivalryHeat>=60" class="preview-history"><small>LO QUE ARRASTRA ESTE PARTIDO</small><strong v-if="rivalryHeat>=60">Rivalidad {{rivalryHeat}}/100</strong><span v-for="item in historyContext" :key="item.key"><b>{{item.title}}</b><em>{{item.season}} · {{item.summary}}</em></span><span v-for="item in reencounterContext.slice(0,2)" :key="`re-${item.player_id}`"><b>Reencuentro: {{item.player_name || item.name}}</b><em>{{item.detail || 'Se enfrenta a una etapa anterior de su carrera.'}}</em></span></div>
          ...
          <div v-if="match.status==='finished'" class="match-post-v2 d7-match-post">
            ...
            <div v-if="match.diagnosis" class="post-diagnosis"><small>DIAGNÓSTICO DEL CUERPO TÉCNICO</small><strong>{{match.diagnosis.verdict}} · {{match.diagnosis.score}}</strong><span v-for="reason in match.diagnosis.reasons || []" :key="reason">{{reason}}</span><em v-for="action in match.diagnosis.next_actions || []" :key="action">→ {{action}}</em><div v-if="(match.diagnosis.consequence_links||[]).length" class="diagnosis-consequences"><small>CONSECUENCIAS Y DESTINOS</small><span v-for="link in match.diagnosis.consequence_links" :key="`${link.workspace}-${link.label}`"><b>{{link.label}}</b><em>{{link.detail}}</em></span></div></div>
            <div v-if="match.committed" class="post-impact"><small>CONSECUENCIAS YA APLICADAS</small><span><b>Clasificación</b><strong>{{postPosition}}</strong></span><span><b>Moral</b><strong>{{postContext.morale_average ?? dashboard.morale_average ?? '—'}}/100</strong></span><span><b>Consejo</b><strong>{{postContext.board_confidence || dashboard.board_confidence || 'A la espera'}}</strong></span><span><b>Siguiente</b><strong>{{nextOpponentName}}</strong><em>{{postNextMatch?.date || 'fecha por confirmar'}}</em></span></div>
            <div v-if="match.committed && nextAbsences.length" class="post-availability"><small>BAJAS PARA EL SIGUIENTE PARTIDO</small><span v-for="row in nextAbsences" :key="`${row.kind}-${row.player_id}`"><b>{{row.player_name || row.name || `Jugador #${row.player_id}`}}</b><strong>{{row.label || row.status || row.kind}}</strong><em>{{row.detail || 'No estará disponible en la próxima convocatoria.'}}</em></span></div>
            <div v-if="roundResults.length" class="post-round-results"><header><span><small>RESULTADOS DE LA COMPETICIÓN</small><strong>{{roundSummary.label}} · {{roundSummary.competition_name}}</strong></span><b>{{roundSummary.result_count}}/{{roundSummary.fixture_count}}</b></header><div><span v-for="row in roundResults" :key="row.fixture_id" :class="{controlled:row.controlled}"><em>{{row.home_team}}</em><strong>{{row.home_goals}}–{{row.away_goals}}</strong><em>{{row.away_team}}</em><i v-if="row.controlled">TU PARTIDO</i></span></div></div>
...
<style scoped>
.preview-history{display:grid;gap:5px;padding:10px;border:1px solid var(--line,#d7dde6);border-radius:9px;background:color-mix(in srgb,var(--f-action,#4c83ff) 5%,var(--surface,#fff))}.preview-history>small{font-size:11px;font-weight:900;letter-spacing:.08em;color:var(--f-action,#4c83ff)}.preview-history>strong{font-size:12px}.preview-history>span{display:grid;gap:1px;padding-top:4px;border-top:1px solid var(--line,#d7dde6)}.preview-history b{font-size:11px}.preview-history em{font-size:11px;font-style:normal;color:var(--text-soft,#687386);line-height:1.35}
.bench-advice-row{display:grid;gap:4px;padding:10px 0;border-bottom:1px solid var(--line,#d7dde6)}.bench-advice-row:last-child{border-bottom:0}.bench-advice-row span,.bench-advice-row em{font-size:11px;color:var(--text-soft,#687386)}.bench-advice-row.high strong{font-weight:900}.performance-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px}.performance-grid span{display:grid;grid-template-columns:1fr auto;gap:2px 8px;padding:7px;border-radius:8px;background:var(--surface-soft,#f5f7fa)}.performance-grid span em{grid-column:1/3;font-size:11px;color:var(--text-soft,#687386)}.post-diagnosis{display:grid;gap:5px;padding:12px;border-radius:10px;background:var(--surface-soft,#f5f7fa)}.post-diagnosis small{font-size:11px;font-weight:900;letter-spacing:.08em}.post-diagnosis span,.post-diagnosis em{font-size:11px}.post-diagnosis em{color:var(--text-soft,#687386)}
...
.post-impact{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;padding:12px;border:1px solid var(--line,#d7dde6);border-radius:10px}.post-impact>small,.post-ratings>small,.post-availability>small{grid-column:1/-1;font-size:11px;font-weight:900;letter-spacing:.08em;color:var(--text-soft,#687386)}.post-impact>span{display:grid;gap:2px}.post-impact b{font-size:11px;color:var(--text-soft,#687386)}.post-impact strong{font-size:12px}.post-impact em{font-size:11px;font-style:normal;color:var(--text-soft,#687386)}.post-availability{display:grid;gap:7px;padding:12px;border:1px solid var(--line,#d7dde6);border-radius:10px}.post-availability>span{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:2px 8px}.post-availability b,.post-availability strong{font-size:11px}.post-availability em{grid-column:1/-1;font-size:11px;font-style:normal;color:var(--text-soft,#687386)}.post-round-results{display:grid;gap:8px;padding:12px;border:1px solid var(--line,#d7dde6);border-radius:10px}.post-round-results>header{display:flex;align-items:start;justify-content:space-between;gap:12px}.post-round-results>header span{display:grid;gap:2px}.post-round-results>header small{font-size:11px;font-weight:900;letter-spacing:.08em;color:var(--text-soft,#687386)}.post-round-results>header strong{font-size:13px}.post-round-results>header>b{font-size:11px;color:var(--text-soft,#687386)}.post-round-results>div{display:grid;grid-template-columns:1fr 1fr;gap:6px}.post-round-results>div>span{display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);gap:7px;align-items:center;padding:8px 9px;border-radius:8px;background:var(--surface-soft,#f5f7fa)}.post-round-results>div>span.controlled{outline:1px solid var(--f-action,#4c83ff)}.post-round-results em{font-size:11px;font-style:normal;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.post-round-results em:last-of-type{text-align:right}.post-round-results strong{font-size:13px}.post-round-results i{grid-column:1/-1;font-size:11px;font-style:normal;font-weight:900;letter-spacing:.08em;color:var(--f-action,#4c83ff)}.post-ratings{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px;padding:12px;border:1px solid var(--line,#d7dde6);border-radius:10px}.post-ratings>span{display:grid;grid-template-columns:1fr auto;gap:2px 6px}.post-ratings>span b{font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.post-ratings>span strong{font-size:13px}.post-ratings>span em{grid-column:1/-1;font-size:11px;font-style:normal;color:var(--text-soft,#687386)}@media(max-width:1000px){.post-impact{grid-template-columns:1fr 1fr}.post-ratings{grid-template-columns:1fr 1fr}.post-round-results>div{grid-template-columns:1fr}}
