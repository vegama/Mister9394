...
const emit = defineEmits(['toggle-starter','open-player','renew','toggle-listing','auto-select','save-selection','open-tactics','set-captain','respond-concern','discipline'])
  ...
  <section class="screen-grid squad-screen redesigned-squad">
    ...
    <aside class="football-panel lineup modern-lineup-card">
      ...
      <section class="dressing-room-card">
        ...
        <div v-if="dressingRoom.social_groups?.length" class="social-group-list"><small>GRUPOS</small><span v-for="group in dressingRoom.social_groups.slice(0,4)" :key="group.label"><b>{{group.label}}</b><em>{{group.member_ids?.length || 0}} jugadores · cohesión {{group.cohesion}}/100</em></span></div>
