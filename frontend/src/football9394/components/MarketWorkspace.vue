...
<template>
  <section class="market-workspace redesigned-market">
    <article class="football-panel market-workflow-panel">
      ...
      <div class="market-workflow-steps">
        <div v-for="(step,index) in flowSteps" :key="step.key" class="market-workflow-step" :class="step.state">
          <span class="workflow-index">{{index+1}}</span>
          <span><small>{{step.label}}</small><strong>{{step.count}}</strong><em>{{flowStateText(step.state)}}</em></span>
          <span class="workflow-owner">{{step.owner}}</span>
        </div>
      </div>
    ...
    <aside class="market-deal-rail">
      ...
      <article class="football-panel negotiation-stack"><header class="simple-panel-head"><span><small>EN CURSO</small><strong>Negociaciones</strong></span><b>{{activeNegotiations.length}}</b></header><div v-for="n in activeNegotiations" :key="n.id" class="negotiation-card" :class="{needsAction:n.status==='countered'}"><strong>{{playerName(n.player_id)}}</strong><span v-if="n.status==='waiting'">Esperando respuesta · {{dateShort(n.response_date)}}</span><template v-else><span>Contraoferta: {{money(n.counter_fee)}} · ficha {{money(n.counter_salary)}}</span><button class="football-button tiny" @click="emit('counter',n)">Responder ahora</button></template><small>{{handlerText(n)}} · {{n.handler_quality_label || 'gestión de mercado'}}<template v-if="n.response_date"> · próxima respuesta {{dateShort(n.response_date)}}</template></small><small>{{n.deal_type==='loan'?'Cesión':'Traspaso'}} · rol {{roleText(n.squad_role)}}<template v-if="n.deal_type==='loan'"> · ficha {{n.loan_wage_share}}%</template><template v-else-if="n.signing_bonus"> · prima {{money(n.signing_bonus)}}</template></small><button type="button" class="football-button tiny secondary" @click="emit('withdraw',n)">Retirar</button></div><div v-if="!activeNegotiations.length" class="rail-empty">No hay negociaciones pendientes.</div></article>
...
@media(max-width:1050px){.market-workspace{grid-template-columns:1fr}.market-workflow-steps{grid-template-columns:repeat(2,1fr)}.market-owner-strip{grid-template-columns:1fr}.market-workflow-head{align-items:stretch;flex-direction:column}.market-workflow-summary{min-width:0}}
