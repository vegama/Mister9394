...
const props = defineProps({
  ...
  planNeed: { type: Object, default: null },
...
const emit = defineEmits([
  ...
  'search','apply-plan','set-plan-decision','watch','scout','inquire','open-player','choose-target','submit','counter','withdraw','accept-offer'
])
  ...
  <section class="market-workspace redesigned-market">
    ...
    <article class="football-panel market-discovery">
      ...
      <div class="market-plan-priorities"><button type="button" v-for="need in (squadPlan.priorities||[]).slice(0,4)" :key="need.slot" @click="emit('apply-plan',need)"><b>{{need.label}}</b><span>{{need.priority}} · {{need.action}}</span><small v-if="need.slot!=='DEPTH'">{{need.count}} efectivos<template v-if="need.average"> · nivel medio {{need.average}}</template></small><small v-else>Faltan {{need.shortage}} para el mínimo</small><em>Buscar perfiles →</em></button></div>
        <div v-if="(squadPlan.expiring||[]).length || (squadPlan.surplus||[]).length || (squadPlan.succession||[]).length" class="market-player-decisions">
          <small class="decision-heading">DECISIONES POR JUGADOR</small>
          <div v-for="row in [...(squadPlan.expiring||[]),...(squadPlan.succession||[]),...(squadPlan.surplus||[])].slice(0,8)" :key="row.player_id" class="player-decision-row">
            <span><b>{{row.name}}</b><em>{{row.reason}}</em></span>
            <select :value="row.decision" @change="emit('set-plan-decision',{playerId:row.player_id,decision:$event.target.value})">
              <option v-for="option in (squadPlan.decision_options||[])" :key="option.key" :value="option.key">{{option.label}}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <article v-if="comparisonTargets.length>=2" class="market-comparison">
        <header><span><small>COMPARACIÓN A/B/C</small><strong>{{planNeed?.label ? `${planNeed.label} · alternativas` : 'Antes de comprometer presupuesto'}}</strong></span><em>{{planNeed?.shortage ? `Cubre un déficit de ${planNeed.shortage} efectivo${planNeed.shortage===1?'':'s'}.` : 'Compara nivel, certeza, encaje y coste de oportunidad'}}</em></header>
        ...
        <p v-if="comparison?.verdict" class="market-comparison-verdict" :class="{undecidable:comparison.undecidable}">
          <strong>{{comparison.undecidable?'Todavía no se puede decidir':'Diferencia clara'}}</strong>
          <span>{{comparison.verdict}}</span>
          <em v-for="a in (comparison.actions||[]).slice(0,2)" :key="`cmp-${a.player_id}-${a.action}`">{{a.reason}}</em>
        </p>
      </article>
    ...
    <aside class="market-deal-rail">
      ...
      <article v-if="(marketFlow.listings||[]).length" class="football-panel negotiation-stack"><header class="simple-panel-head"><span><small>SALIDAS ACTIVAS</small><strong>Jugadores ofrecidos</strong></span><b>{{marketFlow.listings.length}}</b></header><div v-for="listing in marketFlow.listings" :key="listing.player_id" class="negotiation-card"><strong>{{playerName(listing.player_id)}}</strong><span>{{listing.open_offers || 0}} club{{listing.open_offers===1?'':'es'}} interesados</span><small>Precio orientativo {{money(listing.asking_price)}} · valor estimado {{money(listing.estimated_value)}}</small></div></article>
    </aside>
  </section>
</template>
...
.market-player-decisions{display:grid;gap:6px;padding-top:8px;border-top:1px solid var(--line,#d7dde6)}.decision-heading{font-size:11px;font-weight:800;letter-spacing:.08em;color:var(--text-soft,#687386)}.player-decision-row{display:grid;grid-template-columns:minmax(0,1fr) 145px;gap:10px;align-items:center;padding:7px 9px;background:var(--surface,#fff);border-radius:7px}.player-decision-row span{display:grid;gap:1px}.player-decision-row em{font-size:11px;font-style:normal;color:var(--text-soft,#687386)}.player-decision-row select{min-width:0}
.scouting-territory{display:grid;gap:5px;padding-top:8px;border-top:1px solid var(--line,#d7dde6)}.scouting-territory>small{font-size:11px;font-weight:800;letter-spacing:.08em;color:var(--text-soft,#687386)}.scouting-territory>span{display:flex;justify-content:space-between;gap:8px;font-size:11px}.scouting-territory em{font-style:normal;color:var(--text-soft,#687386)}
</style>
