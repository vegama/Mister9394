...
<template>
  <section class="training-workspace">
    <header class="training-hero football-panel">
      ...
      <div class="training-owner" :class="training.responsibility_mode"><small>RESPONSABLE · {{training.responsibility_mode==='delegated'?'DELEGADO':'CONTROL DIRECTO'}}</small><strong>{{training.responsibility?.assignee_name || 'Tú (mánager)'}}</strong><span>{{training.responsibility?.quality_label || 'Decisión directa'}} · carga {{training.responsibility?.workload_label || '—'}}</span><button type="button" class="football-button tiny secondary" @click="emit('open-staff')">Cambiar delegación</button></div>
    </header>
    ...
    <div class="training-grid">
      <article class="football-panel training-plan-card">
        ...
        <div class="training-actions"><p>Los días de partido y la víspera pueden ajustar automáticamente la sesión para evitar incoherencias con el calendario.</p><span class="training-action-buttons"><button v-if="planDirty" type="button" class="football-button secondary" @click="resetPlan">Descartar cambios</button><button type="button" class="football-button primary" :disabled="!planDirty" @click="save">{{planDirty?'Guardar plan':'Plan guardado'}}</button></span></div>
      </article>
