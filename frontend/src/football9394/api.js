...
export const football9394Api={
  ...
  setTrainingRoleFocus:(careerId,playerId,roleFocus)=>footballRequest(`/api/football9394/careers/${careerId}/training/roles/${Number(playerId)}`,{method:'PUT',body:JSON.stringify({role_focus:roleFocus})}),
  ...
  compareCandidates:(careerId,playerIds)=>footballRequest(`/api/football9394/careers/${careerId}/scouting/compare`,{method:'POST',body:JSON.stringify({player_ids:playerIds.map(Number)})}),
  ...
  setSquadPlanDecision:(careerId,playerId,decision)=>footballRequest(`/api/football9394/careers/${careerId}/squad-plan/players/${Number(playerId)}`,{method:'PUT',body:JSON.stringify({decision})}),
