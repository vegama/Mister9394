...
const emit = defineEmits(['close', 'update:tab', 'promise-role'])
...
const ratingLabel = computed(() => { const v=overall.value; return v>=92?'REFERENCIA MUNDIAL':v>=86?'ESTRELLA':v>=80?'ALTO NIVEL':v>=72?'TITULAR DE NIVEL':v>=64?'COMPETITIVO':'DESARROLLO' })
...
const availabilityLabel = computed(() => medical.value.status || p.value.status || 'Disponible')
...
<BaseModal :embedded="embedded" size="full" layer="entity" panel-class="football9394-player-modal player-profile-v2" close-label="Cerrar ficha del jugador" aria-label="Ficha futbolista 1993-94" @close="emit('close')">
  <template #header>
    <div class="player-hero-v2">
      ...
      <div class="player-hero-rating"><small>NIVEL ACTUAL</small><b>{{overall || '—'}}</b><strong>{{ratingLabel}}</strong></div>
  ...
  <div class="f9394-profile-shell player-profile-v2-shell">
    ...
    <section v-else-if="tabModel==='attributes'" class="f9394-attribute-groups">
      <article v-for="group in attributeGroups" :key="group[0]" class="f9394-card"><h3>{{group[0]}}</h3><div v-for="item in group[1]" :key="item[1]" class="f9394-attribute-line"><span>{{item[0]}}</span><div class="f9394-meter"><i :style="{width:`${Number(valueOf(item[1]))||0}%`}"></i></div><b :class="toneFor(valueOf(item[1]))">{{valueOf(item[1])}}</b></div></article>
    </section>
    ...
    <section v-else-if="tabModel==='medical'" class="f9394-card"><h3>HISTORIAL MÉDICO</h3><div class="f9394-medical-banner"><b>{{medical.status||'Disponible'}}</b><span>{{medical.current_injury?.name||'Sin lesión actual'}}</span></div><dl v-if="medical.current_injury" class="f9394-data-grid f9394-current-injury"><div><dt>Zona</dt><dd>{{medical.current_injury.body_area||'—'}}<template v-if="medical.current_injury.laterality"> · {{medical.current_injury.laterality}}</template></dd></div><div><dt>Vuelta prevista</dt><dd>{{medical.current_injury.expected_return||'—'}}</dd></div><div><dt>Días restantes</dt><dd>{{medical.injury_days??'—'}}</dd></div><div><dt>Recaída</dt><dd>{{medical.current_injury.recurrence?'Sí':'No'}}</dd></div></dl><div class="f9394-table-wrap"><table><thead><tr><th>LESIÓN</th><th>ZONA</th><th>LADO</th><th>INICIO</th><th>ALTA</th><th>DÍAS</th><th>RECAÍDA</th></tr></thead><tbody><tr v-for="(injury,index) in medical.history||[]" :key="`${injury.start}-${index}`"><td>{{injury.name}}</td><td>{{injury.body_area||'—'}}</td><td>{{injury.laterality||'—'}}</td><td>{{injury.start||'—'}}</td><td>{{injury.end||injury.expected_return||'—'}}</td><td>{{injury.days??'—'}}</td><td>{{injury.recurrence?'Sí':'No'}}</td></tr><tr v-if="!(medical.history||[]).length"><td colspan="7">Sin lesiones registradas.</td></tr></tbody></table></div></section>

    <section v-else-if="tabModel==='career'" class="f9394-card"><h3>TRAYECTORIA</h3><div class="f9394-table-wrap"><table><thead><tr><th>TEMPORADA</th><th>CLUB</th><th>COMPETICIÓN</th><th>PJ</th><th>G</th><th>A</th></tr></thead><tbody><tr v-for="(row,index) in player.career_seasons||[]" :key="`${row.season}-${index}`"><td>{{row.season}}</td><td>{{player.team_name||'—'}}</td><td>Temporada de carrera</td><td>{{row.appearances??0}}</td><td>{{row.goals??0}}</td><td>{{row.assists??0}}</td></tr><tr v-if="!(player.career_seasons||[]).length"><td colspan="6">La trayectoria de esta carrera se irá archivando al cerrar cada temporada.</td></tr></tbody></table></div></section>

    <section v-else-if="tabModel==='scout'" class="f9394-tab-grid"><div class="f9394-card"><h3>INFORME DEL TÉCNICO / OJEADOR</h3><p class="f9394-report">{{scout.summary||'Todavía no existe un informe suficiente sobre este jugador.'}}</p><dl class="f9394-data-grid"><div><dt>Conocimiento</dt><dd>{{scout.knowledge||'—'}}</dd></div><div><dt>Confianza</dt><dd>{{scout.confidence||'—'}}</dd></div><div><dt>Rol recomendado</dt><dd>{{scout.recommended_role||'—'}}</dd></div><div><dt>Encaje táctico</dt><dd>{{scout.tactical_fit||tacticalFit.label||'—'}}</dd></div></dl></div><div class="f9394-card"><h3>PUNTOS FUERTES</h3><ul><li v-for="item in scout.strengths||[]" :key="item">{{item}}</li><li v-if="!(scout.strengths||[]).length">Consulta los atributos destacados del resumen.</li></ul><h3>PUNTOS DÉBILES</h3><ul><li v-for="item in scout.weaknesses||[]" :key="item">{{item}}</li><li v-if="!(scout.weaknesses||[]).length">Sin datos suficientes.</li></ul></div></section>
  </div>
</BaseModal>
</template>
