...
def main():
    ...
    regby={int(r['source_id']):r for r in registry['players'] if r.get('source_id') is not None};history_by_team_name={(int(p.get('team_id') or 0),norm(p.get('rsssf_name_1993_94'))):int(p['source_id']) for p in players if p.get('rsssf_name_1993_94')};nextid=max(byid)+1
    ...
    for ci,c in enumerate(stage['clubs']):
        ...
        for ri,row in enumerate(c['players']):
            key=(norm(name),norm(row['rsssf_name']));verified_eid=EXISTING.get(key);staged_eid=int(row.get('resolved_source_id') or 0) or None;historical_eid=history_by_team_name.get((tid,norm(row['rsssf_name'])));eid=verified_eid or (staged_eid if staged_eid in byid else None) or (historical_eid if historical_eid in byid else None);p=byid.get(eid) if eid else None
            if verified_eid and not p:raise RuntimeError(f'Missing explicit Greek existing id {verified_eid}')
            if verified_eid:
                role=EXACT_ROLE_BY_EXISTING[verified_eid];role_source='verified_existing_historical_specialist_role'
            else:
            ...
            row.update({'identity_resolution':'reused_verified_greece_pool' if verified_eid else ('reused_staged_identity' if eid else 'created_historical_identity'),'resolved_source_id':sid,'resolved_display_name':p['display_name'],'resolved_primary_role':role,'resolved_exact_position':ROLE_TO_LABEL[role],'position_source':role_source,'resolved_birth_date':p.get('birth_date'),'resolved_country_id':p.get('international_country_id') or p.get('birth_country_id')})
            r=regby.get(sid)
