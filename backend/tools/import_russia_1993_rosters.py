...
('lokomotiv moskva','podpaliy'):9495360,
...
def main():
 ...
 for ci,c in enumerate(stage['clubs']):
  ...
  for idx,row in enumerate(c['players']):
   eid=EXISTING.get((norm(name),norm(row['bdfutbol_name'])));p=byid.get(eid) if eid else None;profile=PROFILE_OVERRIDES.get((norm(name),norm(row['bdfutbol_name'])));role,rs=(int(profile['role']),'bdfutbol_player_profile') if profile else role_for(row,idx,ci*31+idx,p);roles[rs]+=1;broad=ROLE_TO_BROAD[role]
   if p is None:
