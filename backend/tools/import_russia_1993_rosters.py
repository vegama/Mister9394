...
EXISTING={
('spartak moskva','onopko'):9494088,('spartak moskva','mamedov'):9495357,('spartak moskva','nikiforov'):9494081,('spartak moskva','lediakhov'):9494089,('spartak moskva','karpin'):9494084,('spartak moskva','pyatnitsky'):9494083,('spartak moskva','tsymbalar'):9494087,('spartak moskva','beschastnykh'):9494085,('spartak moskva','cherchesov'):2705,('spartak moskva','khlestov'):9494090,('spartak moskva','popov'):515,('spartak moskva','radchenko'):517,
('dynamo moskva','tetradze'):9494086,('dynamo moskva','kovtun'):9495358,
('lokomotiv moskva','podpaliy'):9495360,('lokomotiv moskva','rachimov'):503,
('spartak vladikavkaz','khapov'):9495355,
}
...
XI=(0,1,3,4,2,9,7,7,13,17,17); BASE={1:78,2:75,3:74,4:73,5:73,6:72,7:71,8:69,9:72,10:69,11:68,12:68,13:68,14:67,15:66,16:65,17:65,18:64}
CLUB_ALIAS={'spartak moskva':'Spartak Moskva','rotor volgograd':'Rotor Volgograd','dynamo moskva':'Dynamo Moskva','dinamo moskva':'Dynamo Moskva','tekstilshchik kamyshin':'Tekstilshchik Kamyshin','kamyshin':'Tekstilshchik Kamyshin','lokomotiv moskva':'Lokomotiv Moskva','spartak vladikavkaz':'Spartak Vladikavkaz','torpedo moskva':'Torpedo Moskva','uralmash':'Uralmash','cska moskva':'CSKA Moskva','kamaz':'KAMAZ','zhemchuzhina sochi':'Zhemchuzhina Sochi','zhemchuzhina':'Zhemchuzhina Sochi','dynamo stavropol':'Dynamo Stavropol','lokomotiv nizhny novgorod':'Lokomotiv Nizhny Novgorod','lokomotiv nizhny':'Lokomotiv Nizhny Novgorod','krylia sovetov':'Krylia Sovetov','luch vladivostok':'Luch Vladivostok','vladivostok':'Luch Vladivostok','okean nakhodka':'Okean Nakhodka','rostselmash':'Rostselmash','asmaral moskva':'Asmaral Moskva'}
PROFILE_OVERRIDES={
('lokomotiv moskva','ovchinnikov'):{'display_name':'Sergei Ivanovich Ovchinnikov','first_name':'Sergei Ivanovich','surname1':'Ovchinnikov','birth_date':'1970-11-10T00:00:00','birth_country_id':40,'international_country_id':40,'role':0,'position':'Goalkeeper','bdfutbol_id':'71772','bdfutbol_url':'https://www.bdfutbol.com/en/j/j71772.html'},
('lokomotiv moskva','alenichev'):{'display_name':'Dimitri Anatolievich Alenichev','first_name':'Dimitri Anatolievich','surname1':'Alenichev','birth_date':'1972-10-20T00:00:00','birth_country_id':40,'international_country_id':40,'role':7,'position':'Centre Midfielder','bdfutbol_id':'99450','bdfutbol_url':'https://www.bdfutbol.com/en/j/j99450.html'},
('dynamo moskva','simutenkov'):{'display_name':'Igor Vitalyevich Simutenkov','first_name':'Igor Vitalyevich','surname1':'Simutenkov','birth_date':'1973-04-03T00:00:00','birth_country_id':40,'international_country_id':40,'role':17,'position':'Centre Forward','bdfutbol_id':'1084','bdfutbol_url':'https://www.bdfutbol.com/en/j/j1084.html'},
}
...
def main():
 ...
 regby={int(r['source_id']):r for r in registry['players'] if r.get('source_id') is not None};history_by_team_name={(int(p.get('team_id') or 0),norm(p.get('bdfutbol_name_1993_94'))):int(p['source_id']) for p in players if p.get('bdfutbol_name_1993_94')};nextid=max(byid)+1;used={tuple(int((p.get('attributes') or {}).get(k,-1)) for k in ATTRS) for p in players if p.get('attributes')}
 safe=set(EXISTING.values());mixed=0
 for p in players:
  if int(p.get('team_id') or 0) in {617,1087} and int(p['source_id']) not in safe:
   mixed+=not bool(p.get('retired'));p['retired']=True;p['historical_exclusion_reason']='mixed_era_mdb_not_verified_russia_1993'
 ...
 for ci,c in enumerate(stage['clubs']):
  ...
  for idx,row in enumerate(c['players']):
   verified_eid=EXISTING.get((norm(name),norm(row['bdfutbol_name'])));staged_eid=int(row.get('resolved_source_id') or 0) or None;historical_eid=history_by_team_name.get((tid,norm(row['bdfutbol_name'])))
   ...
   eid=verified_eid or (staged_eid if staged_eid in byid else None) or (historical_eid if historical_eid in byid else None);p=byid.get(eid) if eid else None;profile=PROFILE_OVERRIDES.get((norm(name),norm(row['bdfutbol_name'])));role,rs=(int(profile['role']),'bdfutbol_player_profile') if profile else role_for(row,idx,ci*31+idx,p);roles[rs]+=1;broad=ROLE_TO_BROAD[role]
   if p is None:
    sid=nextid;nextid+=1;display=str(profile['display_name']) if profile else str(row['bdfutbol_name']);first,surname=(profile['first_name'],profile['surname1']) if profile else split_name(display);ov=overall(pos,row)
    p={'source_id':sid,'team_id':tid,'display_name':display,'first_name':first,'surname1':surname,'surname2':None,'birth_date':profile.get('birth_date') if profile else None,'birth_country_id':profile.get('birth_country_id') if profile else None,'international_country_id':profile.get('international_country_id') if profile else None,'preferred_foot':None,'shirt_number':None,'primary_role':role,'broad_position':broad,'overall':ov,'category':ov,'height_cm':None,'weight_kg':None,'salary':0,'release_clause':0,'contract_start_year':1993,'contract_end_year':None,'loan':False,'initially_reserve':int(row.get('starts') or 0)<8,'retired':False,'attributes':{},'role_ratings':ratings(role),'hidden_traits':{'individualist':False,'killer_pass':False,'holds_ball':False,'long_shots':False,'cuts_inside':False,'first_time_play':False,'dives':False},'external_origin':'historical_russia_1993','creation_batch':BATCH,'profile_review_required':False,'nationality_resolution':'unknown_until_source_backed' }
    ...
    used.add(vec);p['attributes']=at;p['attribute_source']='fixed_source_comparable_russia_1993';p['attribute_comparable_source_ids']=comps;players.append(p);byid[sid]=p;created+=1
   else:
   ...
   if r is not None:r.update({'display_name':p['display_name'],'first_name':p.get('first_name'),'surname1':p.get('surname1'),'surname2':p.get('surname2'),'birth_date':p.get('birth_date'),'country_id':p.get('international_country_id') or p.get('birth_country_id'),'country_name':'Rusia' if (p.get('international_country_id') or p.get('birth_country_id'))==40 else None,'broad_position':broad,'team_id':tid,'team_name':name,'creation_batch':p.get('creation_batch'),'identity_source':p['historical_data_source'],'identity_source_url':c['bdfutbol_squad_url'],'verified_national_pool_year':1993 if (p.get('international_country_id') or p.get('birth_country_id'))==40 else None,'historical_position_1993_94':p['historical_position_1993_94'],'historical_club_1994':name,'overall':p.get('overall'),'attribute_source':p.get('attribute_source'),'profile_review_required':False,'duplicate_check':'russia_1993_explicit_identity_gate','matched_existing_id':sid if verified_eid else None,'bdfutbol_search_name':p['display_name'],'bdfutbol_id':str(p.get('bdfutbol_id') or ''),'bdfutbol_url':p.get('bdfutbol_url',''),'photo_filename':f'{sid}.jpg','photo_status':'ready_for_download' if p.get('bdfutbol_id') else 'pending_identity_profile'})
 ...
 for p in players:
  ...
  if sid in resolved or int(p.get('international_country_id') or 0)!=40:continue
 ...
 snap['leagues']=[l for l in snap['leagues'] if int(l.get('source_id') or 0) not in {15,LEAGUE_ID}];snap['leagues'].append({'source_id':LEAGUE_ID,'country_id':40,'country':'Rusia','name':'Supreme League','short_name':'Supreme League','level':1,'team_count':18,'turns':2,'yellow_card_cycle':3,'max_foreigners_starting':None,'max_foreigners_squad':None,'prefer_nationals':False,'source_start':'1993-03-01T00:00:00','source_end':'1993-11-30T00:00:00','source_edition':'1993','admitted':True,'signable':True,'source_rule_hints':{'historical_runtime_id':True,'points_win':2,'direct_relegation_places':[15,16,17,18],'modern_mdb_source_id_blocked':15,'foreign_rule_status':'separate_from_roster_activation'}});snap['leagues'].sort(key=lambda l:int(l.get('source_id') or 0))
 rus=next(l for l in foundation['leagues'] if l['key']=='rus_1993');rus['activation_status']='active_historical_roster_gate_passed';rus['activated_runtime_league_id']=LEAGUE_ID
 for c in rus['clubs']:c['team_id']=TEAM_IDS[c['name']];c['roster_status']='complete_historical_1993'
 counts={};rc={}
 ...
 stranded=[p for p in players if int(p.get('team_id') or 0)==OTHER_RUSSIA_ID and not p.get('retired') and CLUB_ALIAS.get(norm(p.get('historical_club_1994')))]
 if stranded:raise RuntimeError('Stranded: '+','.join(p['display_name'] for p in stranded))
 audit={'schema_version':1,'season':'1993','league_id':LEAGUE_ID,'status':'pass_russia_1993_active','staged_rows':sum(len(c['players']) for c in stage['clubs']),'unique_staged_identities':len({int(r['resolved_source_id']) for c in stage['clubs'] for r in c['players']}),'clubs':18,'minimum_required':18,'source_roster_target':'all rows on pinned BDFutbol season squad pages','minimum_active_roster':min(counts.values()),'roster_counts':counts,'role_counts':rc,'reused_existing_players':reused,'created_players':created,'extra_verified_pool_players_reassigned':len(extras),'extra_reassigned':extras,'mixed_era_memberships_excluded':mixed,'position_provenance':dict(roles),'otros_russia_stranded_recognised_club':len(stranded),'modern_mdb_league_id_15_active':False,'identities':ident,'duplicate_policy':'explicit verified reuse only; no fuzzy surname merging; duplicate BDF surnames remain distinct by club/age/row identity','rating_policy':'fixed source-comparable attributes on the game scale; no football 75/25 rule'}
 if audit['staged_rows']!=audit['unique_staged_identities']:raise RuntimeError('Russia source-roster identity cardinality failed')
