...
def test_full_9394_rollover_builds_playable_9495_with_honours_europe_and_summer_market():
    ...
    assert fixture and fixture['date'].startswith('1994-')
...
def test_rollover_can_repeat_into_9596_without_resetting_history():
    ...
    assert len(snap['honours'])==50
    assert {key:len(value) for key,value in snap['continental_qualifiers'].items()}=={'1':8,'2':16,'90':32}
    assert snap['next_match']['date'].startswith('1995-')
    assert career.play_next_matchday()['completed_matchday']>=1
...
def test_uruguay_odd_team_calendar_keeps_two_historical_round_robins_and_byes():
    ...
    assert len(career.career_calendar())==24
...
def test_selection_is_persistent_and_drives_controlled_team_sheet(tmp_path):
    ...
    # Exchange one field player with an available substitute.
    starter_ids = list(initial['starter_ids'])
    bench_ids = list(initial['bench_ids'])
    incoming = bench_ids[0]
    outgoing_index = next(i for i, pid in enumerate(starter_ids) if career.universe.players_by_id[pid].get('broad_position') != 'POR')
    outgoing = starter_ids[outgoing_index]
    starter_ids[outgoing_index] = incoming
    bench_ids[0] = outgoing
    changed = career.set_selection(starter_ids, bench_ids)
